...
const roundFontSizePx = (px: number) => Math.round(px);
...
export function deriveMono(body: TypographyToken): TypographyValue {
  const size = roundFontSizePx(body.value.fontSize * 0.875);
  return {
...
export function deriveTypeScale(
  ...
  return TYPE_SLOTS.map(({ role, exponent, lineHeight }) => {
    const size = roundFontSizePx(base.fontSize * Math.pow(ratio, exponent));
