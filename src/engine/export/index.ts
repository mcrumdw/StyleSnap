...
import { SCHEMA_VERSION } from "../../contract/schema";
...
function header(input: ExportInput): string {
  ...
  return [
    ...
    `> ${input.rawTokenCount} raw tokens → ${input.tokens.length} reviewed tokens (${input.mergeCount} merge${input.mergeCount === 1 ? "" : "s"}). Schema v${SCHEMA_VERSION}.`,
  ].join("\n");
}
